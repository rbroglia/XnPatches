...
logical::                 forcesRB  = .false. ! Inquiring flag for forces saving in RB format.
...
if (.NOT.is_file) then
  err = File_Not_Found(filename=File_grd,cpn='XnPatches')
else
...
if (.NOT.is_file) then
  err = File_Not_Found(filename=File_icc,cpn='XnPatches')
else
...
if (sol) then
  ...
  if (.NOT.is_file) then
    err = File_Not_Found(filename=File_sol,cpn='XnPatches')
  else
...
enddo
read(unit_icc)Nrcc
write(stdout,'(A)')' RCC dimension '//trim(str(.true.,Nrcc))
...
if (Np==0) then
  ...
  !  if (forcesRB) err = remove_file(adjustl(trim(File_out))//"-forces.RB") ! Riccardo Broglia forces.RB
  !endif
endif
stop
!-----------------------------------------------------------------------------------------------------------------------------------
contains
  ...
  write(stdout,'(A)') '  -forcesB         => compute forces and save also RB outputs (default no, do not compute them)'
  ...
  character(9)::   switch          ! Switch string.
  ...
  do while (c<Nca)
    ...
    case('-forces') ! forces computing
      forces = .true.
    case('-forcesRB') ! forces computing
      forces = .true.
      forcesRB = .true.
